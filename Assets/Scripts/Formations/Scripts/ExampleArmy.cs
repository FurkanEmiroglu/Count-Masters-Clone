...
public class ExampleArmy : MonoBehaviour {
    ...
    private void Awake() {
        _parent = GameObject.Find("MainCharacter").transform;
    }
    ...
    private void Spawn(IEnumerable<Vector3> points) {
        ...
        {
            var unit = objectPooler.SpawnFromPool("CloneCharacter", transform.position + pos, Quaternion.identity, _parent);
    ...
    private void Kill(int num) {
        for (var i = 0; i < num; i++) {
            ...
            objectPooler.ReturnToPool("CloneCharacter",unit);
        }
    }
}